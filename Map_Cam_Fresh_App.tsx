...
function Camera({mode,setMode,gps,s,setS,close}:any){
  ...
  return <View style={st.cam}><CameraView ref={ref} style={StyleSheet.absoluteFill} facing="back" mode={mode}>
    <View style={st.camTop}><Round t="×" f={close}/><View style={st.mode}>
    ...
    </View><Round t="⚙" f={()=>Alert.alert('Camera','Use Settings from Home.')}/></View>

    <View style={st.tools}><Round t="⚡" f={()=>{}}/>
      <Round t={g?'GPS':'GPS…'} f={()=>Alert.alert('GPS',g?g.address:'GPS unavailable')}/></View>

    <View style={st.stamp}><View style={{flex:1}}>
      ...
      <Text style={st.watermark}>{s.watermark}</Text></View>
    ...
    <View style={st.bottom}>{mode==='video'&&<Pressable style={st.audio} onPress={()=>setS((x:Settings)=>({...x,audio:!x.audio}))}>
      <Text style={st.audioTxt}>{s.audio?'🎙 ON':'🔇 OFF'}</Text><Text style={st.audioSmall}>Audio</Text></Pressable>}
      {mode==='photo'?<Pressable style={st.shutter} onPress={take} disabled={busy}><View style={st.shutterInner}/></Pressable>:
...
const st=StyleSheet.create({
  ...
  cam:{flex:1,backgroundColor:'#000'},camTop:{paddingTop:52,paddingHorizontal:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},
