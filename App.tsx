...
function Camera({mode,setMode,gps,s,setS,close}:any){
  ...
  async function take(){
    ...
    if(!ref.current || !ready){Alert.alert('Camera','Camera is still starting. Please wait a moment.');return;}
    setBusy(true);
    try{
    ...
    }catch(e){Alert.alert('Camera','Photo could not be captured. Please try again.')}
  ...
  if(!perm?.granted) return <SafeAreaView style={st.safe}><View style={st.center}>
    <View style={st.simpleTop}><Pressable onPress={close}><Text style={st.back}>‹</Text></Pressable><Text style={st.page}>Camera</Text><View style={{width:42}}/></View>
  ...
  return <SafeAreaView style={st.cameraScreen}>
    <View style={st.cameraHeader}>
      <Pressable onPress={close} style={st.backBtn}><Text style={st.backBtnText}>‹</Text></Pressable>
      <View style={st.mode}>
        <Pressable onPress={()=>setMode('photo')} style={[st.modeBtn,mode==='photo'&&st.active]}><Text style={st.modeText}>Photo</Text></Pressable>
        <Pressable onPress={()=>setMode('video')} style={[st.modeBtn,mode==='video'&&st.active]}><Text style={st.modeText}>Video</Text></Pressable>
      </View>
      <Pressable onPress={()=>Alert.alert('Camera Settings','Use Settings from the Map Cam home screen.')} style={st.backBtn}>
        <Text style={st.settingsIcon}>⚙</Text>
      </Pressable>
    </View>
    ...
    <View style={st.previewBox}>
      ...
      />
      {!ready && <View style={st.loadingOverlay}><Text style={st.loadingText}>Starting camera…</Text></View>}
    </View>

    <View style={st.cameraInfo}>
      <View style={st.infoText}>
        {s.siteName&&<Text style={st.stampSite}>{s.siteName}</Text>}
        <Text style={st.gpsLine}>● {g?'GPS READY':'WAITING FOR GPS'}</Text>
        {s.showAddress&&g&&<Text style={st.infoLine}>📍 {g.address}</Text>}
        {s.showCoordinates&&g&&<Text style={st.infoLine}>Lat {g.latitude.toFixed(6)} • Long {g.longitude.toFixed(6)}</Text>}
        {s.showDate&&<Text style={st.infoLine}>🗓 {new Date().toLocaleString()}</Text>}
      </View>
      {s.showMap&&g&&<MapView
        style={st.mini}
        initialRegion={{latitude:g.latitude,longitude:g.longitude,latitudeDelta:.004,longitudeDelta:.004}}
    ...
    <View style={st.cameraBottom}>
      {mode==='video' ? (
        <Pressable style={st.audioControl} onPress={()=>setS((x:Settings)=>({...x,audio:!x.audio}))}>
          <Text style={st.audioTxt}>{s.audio?'🎙 Audio ON':'🔇 Audio OFF'}</Text>
        </Pressable>
      ) : <View style={{width:100}}/>}

      {mode==='photo'
        ? <Pressable style={st.shutter} onPress={take} disabled={busy||!ready}>
            <View style={st.shutterInner}/>
          </Pressable>
        : <Pressable style={[st.record,rec&&st.recording]} onPress={video} disabled={busy&&!rec}>
            <View style={st.recordInner}/>
          </Pressable>}

      <View style={st.readyLabel}><Text style={st.readyText}>{busy?'Saving…':ready?'Ready':'Loading'}</Text></View>
    </View>
  </SafeAreaView>;
}
...
const st=StyleSheet.create({
  ...
  cameraHeader:{height:74,backgroundColor:C.brown,flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:12},
  backBtn:{width:48,height:48,borderRadius:24,backgroundColor:'#FFF9EF',alignItems:'center',justifyContent:'center'},
  backBtnText:{fontSize:34,color:C.brown,lineHeight:38},
  settingsIcon:{fontSize:22,color:C.brown},
  modeText:{fontWeight:'800',color:C.dark},
  previewBox:{flex:1,backgroundColor:'#111',overflow:'hidden'},
  preview:{flex:1},
  loadingOverlay:{position:'absolute',left:0,right:0,top:0,bottom:0,alignItems:'center',justifyContent:'center',backgroundColor:'rgba(0,0,0,.25)'},
  loadingText:{color:'#fff',fontSize:16,fontWeight:'800'},
  cameraInfo:{minHeight:118,maxHeight:145,backgroundColor:C.card,flexDirection:'row',padding:10,borderTopWidth:1,borderTopColor:C.line},
  infoText:{flex:1,justifyContent:'center',paddingRight:8},
  gpsLine:{fontSize:12,fontWeight:'900',color:C.brown,marginBottom:4},
  infoLine:{fontSize:11,color:C.dark,marginBottom:3},
  cameraBottom:{height:112,backgroundColor:C.brown,flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:20},
  audioControl:{backgroundColor:'#FFF9EF',borderRadius:18,paddingHorizontal:12,paddingVertical:9,minWidth:100,alignItems:'center'},
  readyLabel:{width:100,alignItems:'center'},
  readyText:{color:'#FFF9EF',fontWeight:'800',fontSize:12},
  simpleTop:{width:'100%',flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:30},
